...
/**
 * OpenAPI ↔ implementation parity for the HIS-01 surface. The hand-written contract
 * (specs/010-history/contracts/history.openapi.yaml) and the generated document must
 * expose exactly the same set of `history` endpoints.
 */
describe('History OpenAPI parity', () => {
  ...
  function normalize(path: string): string {
    const idx = path.indexOf('history');
    return idx === -1 ? path : path.slice(idx);
  }

  function contractEndpoints(): Set<string> {
    const file = resolve(__dirname, '../../../specs/010-history/contracts/history.openapi.yaml');
  ...
  it('exposes exactly the history endpoints declared in the contract', () => {
