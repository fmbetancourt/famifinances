...
/**
 * Polish (T027) · OpenAPI ↔ implementation parity for the CAT-01 surface. The
 * hand-written contract (specs/005-categories/contracts/categories.openapi.yaml)
 * and the generated document must expose the same set of `categories` endpoints.
 */
describe('Categories OpenAPI parity (Polish)', () => {
  ...
  /** Normalizes any path to its `categories...` suffix so prefixes/versions don't matter. */
  function normalize(path: string): string {
    const idx = path.indexOf('categories');
    return idx === -1 ? path : path.slice(idx);
  }

  function contractEndpoints(): Set<string> {
    const file = resolve(__dirname, '../../../specs/005-categories/contracts/categories.openapi.yaml');
  ...
  it('exposes exactly the category endpoints declared in the contract', () => {
