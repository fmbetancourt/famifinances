...
/**
 * Polish (T026) · OpenAPI ↔ implementation parity for the ACC-01 surface. The
 * hand-written contract (specs/004-accounts/contracts/accounts.openapi.yaml) and
 * the generated document must expose the same set of `accounts` endpoints.
 */
describe('Accounts OpenAPI parity (Polish)', () => {
  ...
  /** Normalizes any path to its `accounts...` suffix so prefixes/versions don't matter. */
  function normalize(path: string): string {
    const idx = path.indexOf('accounts');
    return idx === -1 ? path : path.slice(idx);
  }

  function contractEndpoints(): Set<string> {
    const file = resolve(__dirname, '../../../specs/004-accounts/contracts/accounts.openapi.yaml');
  ...
  it('exposes exactly the account endpoints declared in the contract', () => {
