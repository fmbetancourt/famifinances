...
/**
 * US3 · OpenAPI ↔ implementation parity for the BUD-01 surface. The hand-written
 * contract (specs/008-budget/contracts/budgets.openapi.yaml) and the generated
 * document must expose exactly the same set of `budgets` endpoints.
 */
describe('Budgets OpenAPI parity (US3)', () => {
  ...
  function normalize(path: string): string {
    const idx = path.indexOf('budgets');
    return idx === -1 ? path : path.slice(idx);
  }

  function contractEndpoints(): Set<string> {
    const file = resolve(__dirname, '../../../specs/008-budget/contracts/budgets.openapi.yaml');
  ...
  it('exposes exactly the budget endpoints declared in the contract', () => {
