...
/**
 * Polish · OpenAPI ↔ implementation parity for the UX-01 capture surface. The
 * hand-written contract (specs/013-capture-templates/contracts/capture.openapi.yaml)
 * and the generated document must expose exactly the same set of `capture-*` endpoints.
 */
describe('Capture OpenAPI parity (Polish)', () => {
  ...
  function normalize(path: string): string {
    const idx = path.indexOf('capture');
    return idx === -1 ? path : path.slice(idx);
  }

  function contractEndpoints(): Set<string> {
    const file = resolve(__dirname, '../../../specs/013-capture-templates/contracts/capture.openapi.yaml');
  ...
  it('exposes exactly the capture endpoints declared in the contract', () => {
