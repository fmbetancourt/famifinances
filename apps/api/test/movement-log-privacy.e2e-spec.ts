...
/** Polish (T032) · FR-016 / SC-009: no monetary amount or note appears in logs. */
describe('No financial data in logs (Polish)', () => {
  ...
  it('never writes amounts or note content to stdout/stderr across record/list/edit/delete', async () => {
    ...
    const { member, accountId } = await setupMemberWithAccount(app, mail, 'mv-log@example.com');
    ...
    try {
      ...
      await request(http)
        ...
        .send({ amount: 111222 });
      await request(http).get('/api/v1/movements').set('Authorization', `Bearer ${member.accessToken}`);
