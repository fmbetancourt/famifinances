...
/**
 * Polish (T033) · OpenAPI ↔ implementation parity for the TXN-01 surface. The
 * hand-written contract (specs/006-movements/contracts/movements.openapi.yaml)
 * and the generated document must expose the same set of `movements` endpoints.
 */
describe('Movements OpenAPI parity (Polish)', () => {
  ...
  function normalize(path: string): string {
    const idx = path.indexOf('movements');
    return idx === -1 ? path : path.slice(idx);
  }

  function contractEndpoints(): Set<string> {
    const file = resolve(__dirname, '../../../specs/006-movements/contracts/movements.openapi.yaml');
  ...
  function generatedEndpoints(): Set<string> {
    ...
    for (const [path, item] of Object.entries(doc.paths)) {
      ...
      if (!normalized.startsWith('movements')) {
        continue;
      }
  ...
  it('exposes exactly the movement endpoints declared in the contract', () => {
