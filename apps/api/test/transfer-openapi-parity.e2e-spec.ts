...
/**
 * Polish (T031) · OpenAPI ↔ implementation parity for the TXN-02 surface. The
 * hand-written contract (specs/007-transfers/contracts/transfers.openapi.yaml)
 * and the generated document must expose the same set of `transfers` endpoints.
 */
describe('Transfers OpenAPI parity (Polish)', () => {
  ...
  function normalize(path: string): string {
    const idx = path.indexOf('transfers');
    return idx === -1 ? path : path.slice(idx);
  }

  function contractEndpoints(): Set<string> {
    const file = resolve(__dirname, '../../../specs/007-transfers/contracts/transfers.openapi.yaml');
  ...
  function generatedEndpoints(): Set<string> {
    ...
    for (const [path, item] of Object.entries(doc.paths)) {
      ...
      if (!normalized.startsWith('transfers')) {
        continue;
      }
  ...
  it('exposes exactly the transfer endpoints declared in the contract', () => {
